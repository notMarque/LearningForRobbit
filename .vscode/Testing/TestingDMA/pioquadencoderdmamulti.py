...
class PIOQuadEncoder:
    ...
    def __init__(self, encoderPins: list, positionMemory: bool):
        ...
        self.posRegs = uctypes.struct(uctypes.addressof(self.posRegs), PosRegistersStruct)  # Fill the bytearray with the appropriate struct
        
        ## Creating ustruct to store position values in
        self.posStruct = bytearray(12)   # Allocate 12 bytes for storing positions
        self.posStruct = uctypes.struct(uctypes.addressof(self.posStruct, PositionStruct))
        ...
        # - Chain to the restart DMA which just moves some random data about and then chains back to control (could ping-pong but not worth the one clock cycle)
        ...
        # Follower:
        # - Read: doesn't matter as it will be reset by commander (known safe position is last value in posRegs)
        ...
        self.posRegs.read1 = PIO_BASES[0] + TXF0_ADDR + 4 * 0
        self.posRegs.read2 = PIO_BASES[0] + TXF0_ADDR + 4 * 1
        self.posRegs.read3 = PIO_BASES[0] + TXF0_ADDR + 4 * 2
        # Set Write positions as correct offsets in the positions ucstruct
        self.posRegs.write1 = uctypes.addressof(self.posStruct)
        self.posRegs.write2 = uctypes.addressof(self.posStruct + 4)
        self.posRegs.write3 = uctypes.addressof(self.posStruct + 8)
