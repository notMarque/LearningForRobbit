...
class PIOQuadEncoder:
    ...
    def __init__(self, encoderPins: list):
        ...
        self.timedDmas = list()                 # Creates a list to store all the DMA channels that will be called periodically
        self.chainedToDmas = list()             # Creates a list to store all the DMA channels that will be chained to from the periodic ones
        ...
        # Timer0 register
        # Timer Period set by (X/Y)*sys_clk
        self.velocityPeriod = 5                         # Desired period in milliseconds
        self.velocityHz = 1000 / self.velocityPeriod    # Desired Hz of velocity checking
        self.sys_clk_T = (125 * 1_000_000)      # standard systemclock
        self.X = 1
        self.Y = 
        (0x50000000 + 0x420) = 0b010101101
        ...
        while(i < self.usedSMs):
            ...
            # Adding instances for calculating velocity
            self.timedDmas.append(DMA())        # Appending DMA Onject to list
            self.chainedToDmas.append(DMA())    # Appending DMA objects to list
            self.lastPositions.append(bytearray())      # Append bytearray for storing most recent past position
            self.lasterPositions.append(bytearray())    # Append bytearray for storing older positions
            ...
            controlTimed = self.timedDmas[i].pack_ctrl(inc_read=False, inc_write=False, treq_sel=(0x3b), bswap=False, chain_to=self.chainedToDmas[i].channel)
